...
export default function Home() {
  ...
  const [reelData, setReelData] = useState<GithubReelData | null>(null);
  ...
  useEffect(() => {
    fetch(
      "/api/github?endpoint=" +
        encodeURIComponent("/repos/Avishekguuupta4518/githubreel"),
    )
      .then((res) => res.json())
    ...
    const validateToken = async () => {
      ...
      try {
        const res = await fetch(
          "/api/github?endpoint=" + encodeURIComponent("/user"),
          {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err.message || "Failed to generate reel.";
  ...
  if (showReel && reelData) {
    return (
      <StoryContainer data={reelData} onComplete={() => setShowReel(false)} />
    );
  }
  ...
  const RateLimitPopup = () => {
    if (!showRateLimitPopup || error?.type !== "rate_limit") return null;
    ...
    return (
      <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/60 backdrop-blur-sm">
        ...
        >
          {/* 🔥 YOUR EXACT ERROR UI (UNCHANGED STYLE) */}
          <div
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <button
                  ...
                  className="mt-3 text-xs text-hero-purple hover:underline flex items-center gap-1"
                >
                  Close
                </button>
              </div>
  ...
  return (
    <div
      className={`min-h-[100dvh] flex flex-col items-center justify-center p-6 overflow-hidden relative transition-colors duration-300 ${isDark ? "bg-black text-white" : "bg-white text-black"}`}
    >
      <RateLimitPopup />
      <div className="fixed top-4 right-4 z-50 flex items-center gap-2">
        <a
          href="https://github.com/Avishekguuupta4518"
          target="_blank"
        ...
        <a
          href="https://twitter.com/avishek_gupta45"
          target="_blank"
          rel="noopener noreferrer"
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${isDark ? "bg-neutral-900 text-neutral-300 border-neutral-800 hover:bg-neutral-800" : "bg-neutral-100 text-neutral-700 border-neutral-200 hover:bg-neutral-200"}`}
        >
      ...
      <div
        className={`absolute top-[-20%] left-[-20%] w-[500px] h-[500px] rounded-full blur-[120px] pointer-events-none ${isDark ? "bg-hero-blue/20" : "bg-hero-blue/10"}`}
      />
      <div
        className={`absolute bottom-[-20%] right-[-20%] w-[500px] h-[500px] rounded-full blur-[120px] pointer-events-none ${isDark ? "bg-hero-purple/20" : "bg-hero-purple/10"}`}
      />
      ...
      >
        <div className="text-center mb-12">
          ...
          <p
            className={`font-sans tracking-widest text-sm uppercase ${isDark ? "text-neutral-400" : "text-neutral-500"}`}
          >
            Turn Your GitHub Into Cinema
          </p>
          <p
          ...
          >
            Experience your coding journey through animated stories,
            contribution insights and cinematic visuals.
          </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-center gap-2">
            {session ? (
              <div
                className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm border ${isDark ? "bg-neutral-900 border-neutral-700 text-neutral-300" : "bg-neutral-100 border-neutral-200 text-neutral-700"}`}
              >
                {session.user?.image && (
                  <img
                ...
                <button
                  onClick={() => signOut()}
                  className={`ml-1 p-1 rounded transition-colors ${isDark ? "hover:bg-neutral-700 text-neutral-400 hover:text-neutral-200" : "hover:bg-neutral-200 text-neutral-500 hover:text-neutral-700"}`}
                >
                  <LogOut size={12} />
                </button>
              </div>
                  ...
                  >
                    <div className="pt-2 space-y-2">
                      ...
                      <div className="flex items-start gap-2 text-neutral-600 text-xs p-2">
                        ...
                        <p>
                          Token stays in your browser. Enables: private repos,
                          org repos, 5000 API calls/hr.{" "}
                          <a
                            href="https://github.com/settings/tokens/new?scopes=repo,read:org,read:user&description=GithubReel%202025"
                            target="_blank"
                            rel="noopener noreferrer"
          ...
          >
            {isLoading ? (
              <>
                <Loader2 className="animate-spin" /> Generating Reel...
              </>
            ) : (
              <>
                Play Reel{" "}
          ...
          {/* Trust Indicators */}
          <div className="flex justify-center gap-4 mt-5 text-[11px] text-neutral-500">
        ...
        <div className="mt-12 opacity-200 text-center text-xs text-neutral-600 font-mono">
          <p>CINEMATIC DEVELOPER EXPERIENCE</p>
          <p className="mt-2 opacity-200">
            Best Experienced on Mobile • Try "demo"
          </p>
        </div>
      </motion.div>
    </div>
