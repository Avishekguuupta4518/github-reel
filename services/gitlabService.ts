...
export const fetchGitLabUserStory = async (
  ...
  try {
    ...
    if (userRes.status === 401) {
      throw new Error(
        "Invalid GitLab token. Please check your token and try again.",
      );
    }
    if (!userRes.ok) {
    ...
    const [projectsRes, eventsRes1, eventsRes2, eventsRes3] = await Promise.all(
      [
        ...
        fetch(
          `${GITLAB_API_BASE}/users/${user.id}/projects?per_page=100&order_by=last_activity_at`,
          { headers },
        ),
        // User's events - Pages 1-3
        fetch(
          `${GITLAB_API_BASE}/users/${user.id}/events?per_page=100&after=2025-01-01&page=1`,
          { headers },
        ),
        fetch(
          `${GITLAB_API_BASE}/users/${user.id}/events?per_page=100&after=2025-01-01&page=2`,
          { headers },
        ),
        fetch(
          `${GITLAB_API_BASE}/users/${user.id}/events?per_page=100&after=2025-01-01&page=3`,
          { headers },
        ),
      ],
    );
