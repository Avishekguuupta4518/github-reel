...
export function getTopLanguages(
  langMap: Map<string, LanguageScore>,
  topN: number = 3,
): LanguageScore[] {
...
export function calculateRepoScore(repo: any): number {
  ...
  score += Math.min(
    repo.watchers_count * config.watchersMultiplier,
    config.watchersMax,
  );
...
export function calculateArchetype(
  ...
  const prPercentage =
    totalActivity > 0 ? (breakdown.prs / totalActivity) * 100 : 0;
  const reviewPercentage =
    totalActivity > 0 ? (breakdown.reviews / totalActivity) * 100 : 0;
  const issuePercentage =
    totalActivity > 0 ? (breakdown.issues / totalActivity) * 100 : 0;
  ...
  const weekendPercentage =
    totalWeekCommits > 0 ? (weekendCommits / totalWeekCommits) * 100 : 0;
  ...
  if (productivity.peakHour >= 5 && productivity.peakHour <= 11)
