...
export const CommunitySlide: React.FC<{ data: GithubReelData }> = ({
  data,
}) => {
    ...
    <SlideLayout gradientStart="#be123c" gradientEnd="#881337">
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        <div className="grid grid-cols-1 gap-12 w-full max-w-md">
          ...
          >
            <div
              className={`flex items-center gap-4 mb-2 ${isDark ? "text-white" : "text-black"}`}
            >
              <Users size={48} className="text-rose-400" />
              <span className="text-7xl font-serif italic font-bold">
                {community.followers.toLocaleString()}
              </span>
            </div>
            <span
              className={`font-mono tracking-widest uppercase text-sm ${isDark ? "text-neutral-400" : "text-neutral-600"}`}
            >
              Followers inspired
            </span>
          </motion.div>
        ...
        >
          <div className="flex -space-x-3">
            <div
              className={`w-8 h-8 rounded-full bg-blue-500 border-2 ${isDark ? "border-black" : "border-white"}`}
            />
            <div
              className={`w-8 h-8 rounded-full bg-green-500 border-2 ${isDark ? "border-black" : "border-white"}`}
            />
            <div
              className={`w-8 h-8 rounded-full bg-purple-500 border-2 ${isDark ? "border-black" : "border-white"}`}
            />
          </div>
          <span
            className={`text-sm font-sans ${isDark ? "text-white" : "text-black"}`}
          >
