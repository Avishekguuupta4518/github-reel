...
export const PosterSlide: React.FC<{ data: GithubReelData }> = ({ data }) => {
  ...
  useEffect(() => {
    ...
    // split colors equally for left & right
    const leftColors = ["#FF4D6D", "#FFD166", "#06D6A0", "#4D96FF"];

    const rightColors = ["#9B5DE5", "#F15BB5", "#00BBF9", "#FFD166"];

    const frame = () => {
      // LEFT SIDE (equal particles + colors)
      confetti({
        particleCount: 3,
        angle: 60,
        spread: 65,
      ...
      // RIGHT SIDE (equal particles + colors)
      confetti({
        particleCount: 3,
        angle: 120,
        spread: 65,
  ...
  const generateImage = async (skipFonts = false): Promise<void> => {
    ...
    try {
      ...
      link.download = `githubreel-${data.username}-${data.year}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
      if (!skipFonts) {
  ...
  return (
    <SlideLayout gradientStart="#000000" gradientEnd="#171717">
      <div className="flex-1 flex flex-col items-center justify-center h-full">
        ...
        >
          <div
            className={`flex justify-between items-start border-b pb-4 ${isDark ? "border-neutral-700" : "border-neutral-300"}`}
          >
            <img
              src={data.avatarUrl}
              alt="Avatar"
            ...
            <div className="text-right">
              <h1
                className={`text-3xl font-serif italic ${isDark ? "text-white" : "text-black"}`}
              >
                GithubReel
              </h1>
              <div
                className={`text-xl font-mono tracking-tighter ${isDark ? "text-neutral-500" : "text-neutral-600"}`}
              >
                2025
              </div>
            </div>
          ...
          <div className="space-y-6 py-4">
            <div>
              <div
                className={`text-xs uppercase font-mono tracking-widest mb-1 ${isDark ? "text-neutral-500" : "text-neutral-600"}`}
              >
                Starring
              </div>
              <div
                className={`text-2xl font-sans font-bold ${isDark ? "text-white" : "text-black"}`}
              >
                @{data.username}
              </div>
              <div className="text-lg text-hero-blue font-serif italic">
                {data.archetype}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div
                  className={`text-xl font-serif italic ${isDark ? "text-white" : "text-black"}`}
                >
                  {data.totalCommits}
                </div>
              </div>
            ...
            <div>
              ...
              <div
                className={`text-lg font-bold truncate ${isDark ? "text-neutral-200" : "text-neutral-800"}`}
              >
                {data.topRepo.name}
              </div>
            </div>
          ...
          <div
            className={`border-t pt-4 flex justify-between items-end ${isDark ? "border-neutral-700" : "border-neutral-300"}`}
          >
            <div
              className={`barcode h-8 w-24 opacity-80 ${isDark ? "bg-white" : "bg-black"}`}
              style={{
                backgroundImage: isDark
                  ? "repeating-linear-gradient(90deg, black 0, black 2px, transparent 2px, transparent 4px)"
                  : "repeating-linear-gradient(90deg, white 0, white 2px, transparent 2px, transparent 4px)",
              }}
            ></div>
            <div
              className={`text-[10px] font-mono uppercase ${isDark ? "text-neutral-600" : "text-neutral-500"}`}
            >
              Directed by You
            </div>
          </div>
            ...
            <AnimatePresence>
              {showShareMenu && (
                ...
                >
                  ...
                  <div
                    className={`my-1 border-t ${isDark ? "border-neutral-700" : "border-neutral-200"}`}
                  />
                  <button
                    onClick={() => {
