...
export const ProductivitySlide: React.FC<{ data: GithubReelData }> = ({
  data,
}) => {
  ...
  const getIcon = () => {
    if (productivity.timeOfDay === "Morning")
      return <Sunrise size={100} className="text-orange-400" />;
    if (productivity.timeOfDay === "Afternoon")
      return <Sun size={100} className="text-yellow-400" />;
    if (productivity.timeOfDay === "Evening")
      return <Sunset size={100} className="text-indigo-400" />;
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center">
        ...
        >
          ...
          >
            Peak:{" "}
            {new Date(
              `1970-01-01T${String(productivity.peakHour).padStart(2, "0")}:00:00`,
            ).toLocaleTimeString([], {
              hour: "numeric",
              minute: "2-digit",
              hour12: true,
        ...
        >
          <p
            className={`text-sm font-sans mb-2 tracking-widest uppercase ${isDark ? "text-neutral-400" : "text-neutral-600"}`}
          >
            Your Developer Archetype
          </p>
          <h2
            className={`text-5xl font-bold font-serif italic text-transparent bg-clip-text bg-gradient-to-r ${isDark ? "from-white to-white/60" : "from-black to-black/60"}`}
          >
            {archetype}
          </h2>
        </motion.div>
